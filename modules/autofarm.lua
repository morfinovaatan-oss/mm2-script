-- [[ MM2 AUTO‑FARM – Linoria-based FoxAutofarm Menu ]] --
local Autofarm = {}
...
function Autofarm.Init(GlobalConfig, UI, Lang)
    ...
    -- Кнопка открывает меню FoxAutofarm (на основе LinoriaLib)
    FarmTab:AddButton(text.LaunchBtn, function()
        local success, err = pcall(function()
            -- Загружаем и выполняем скрипт FoxAutofarm
            local scriptContent = [[
                -- Скопированный код, начинающийся с local Players = ... и заканчивающийся Library:Notify(...)
                -- Заменяем заголовок окна и водяной знак
                ...
                local function onPlayerAdded(player: Player) -- Fires on Player joined
                    ...
                    pcall(function() syn.protect_gui(highlight) end)
                ...
                pcall(function() syn.protect_gui(GunHighlight) end)
                pcall(function() syn.protect_gui(GunHandleAdornment) end)
                ...
                return Library:Notify("[FoxAutofarm] Loaded")
            ]]
            loadstring(scriptContent)()
        end)
        if not success then
            warn("Failed to launch FoxAutofarm: " .. tostring(err))
        end
    end)
end
